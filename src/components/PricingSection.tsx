        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto items-stretch">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative flex flex-col rounded-xl p-8 md:p-10 lg:p-12 transition-all duration-500 ${
                plan.highlighted
                  ? 'bg-[#131313] border border-[var(--gold)]'
                  : 'bg-[#111111] border border-[#1a1a1a] hover:border-[#333333]'
              }`}
            >
              {/* Popular Badge */}
              {plan.badge && (
                <div className="absolute -top-3.5 right-8 px-5 py-1.5 bg-[var(--gold)] text-[#0a0a0a] text-[10px] font-bold uppercase tracking-[0.2em] rounded-full">
                  {plan.badge}
                </div>
              )}
              ...
              <div className="mb-8">
                <span
                  className="text-5xl md:text-6xl font-bold text-[var(--gold)]"
                  style={{ fontFamily: "'Playfair Display', serif" }}
                >
                  {plan.price}
                </span>
                <span className="text-[#555555] text-sm ml-3">one-time</span>
              </div>

              {/* Divider */}
              <div className={`w-full h-px mb-8 ${plan.highlighted ? 'bg-[var(--gold)]/20' : 'bg-[#1a1a1a]'}`} />

              {/* Features */}
              <ul className="flex flex-col gap-4 mb-10 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-3 text-sm md:text-base text-white/80">
                    <svg className="w-5 h-5 text-[var(--gold)] flex-none mt-0.5" viewBox="0 0 20 20" fill="currentColor">
              ...
              <a
                ...
                className={`w-full py-4 rounded font-semibold text-sm tracking-wide text-center transition-colors duration-300 block ${
                  plan.highlighted
                    ? 'bg-[var(--gold)] text-[#0a0a0a] hover:bg-[#b0913c]'
                    : 'border border-[var(--gold)] text-[var(--gold)] hover:bg-[var(--gold)] hover:text-[#0a0a0a]'
                }`}
              >
                Get Started on WhatsApp &rarr;
              </a>
            </motion.div>
          ))}
        </div>
